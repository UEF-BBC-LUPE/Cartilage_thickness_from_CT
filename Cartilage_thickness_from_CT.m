...
% CHECK THIS ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !
% load mapping image from biomomentum
biomomentum=imread('map_18LT.PNG');
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
Dicoms_x=flipud(permute(Dicoms,[3 1 2]));
Dicoms_y=flipud(permute(Dicoms,[3 2 1]));
...
% dcm2=flipud(dcm2);
% dcm2=imrotate(dcm2,180);
dicom_mask = mean(Dicoms,3);
% dicom_mask = mean(dcm2,3);

% resize the biomomentum grid image to correspond ẃith the CT image
biomom=imresize(biomomentum,size(dicom_mask));

figure(1);
image(biomom)
hold on
colormap jet
% use alphadata to make the CT image transparent to see the biomomentum
% grid under it
imagesc(dicom_mask,'alphadata',0.5)
axis equal
axis tight
...
%Preallocating to save speed (With 2.08s, without, 2.56s on i5-6267U processor)
temp = imresize(imread([num2str(path) f dicomnames(1).name]),0.5);
Dicoms= int16(zeros(size(temp,1),size(temp,2), length(dicomnames)));

for i = 1:length(dicomnames)
    Dicoms(:,:,i)= imresize(imread([num2str(path) f dicomnames(i).name]),0.5);
    waitbar(i/length(dicomnames));
end
close(h);
